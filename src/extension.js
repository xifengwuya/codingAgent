...
export function activate(context) {
    // 设置 AI 服务提供商
    const setProviderCmd = vscode.commands.registerCommand('extension.setAiProvider', async () => {
    ...
    // 设置 DeepSeek Token
    const setDeepSeekCmd = vscode.commands.registerCommand('extension.setDeepSeekToken', async () => {
        await vscode.commands.executeCommand('workbench.action.openSettings', 'coding-agent-chat.deepseekToken');
    });
    // 设置阿里云 API Key
    const setAliyunCmd = vscode.commands.registerCommand('extension.setAliyunApiKey', async () => {
        await vscode.commands.executeCommand('workbench.action.openSettings', 'coding-agent-chat.aliyunApiKey');
    });
    // 测试当前 API
    const testApiCmd = vscode.commands.registerCommand('extension.testCurrentApi', async () => {
        try {
    ...
    // 测试代理连接
    const testProxyCmd = vscode.commands.registerCommand('extension.testProxy', async () => {
        try {
    ...
    // 打开聊天面板命令
    const showChatPanelCmd = vscode.commands.registerCommand('extension.showChatPanel', async () => {
        try {
            // 强制延迟100ms（避免扩展进程未就绪）
            ...
            vscode.window.showErrorMessage(`打开聊天面板失败: ${error.message}`);
    ...
    context.subscriptions.push(setProviderCmd, setDeepSeekCmd, setAliyunCmd, testApiCmd, testProxyCmd, showChatPanelCmd);
}
